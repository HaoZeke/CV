...
\raggedleft
{\sffamily\bfseries\color{cvCoral}Three-page grant CV}\\[-0.1em]
\headerline{Tailored for IRF/Project Grant review}\\[0.35em]
...
\metric{11} peer-reviewed or accepted selected papers \quad
...
\compactentry{IPAM Fellow, Institute for Pure and Applied Mathematics (UCLA)}{2021}{Tensor Methods and Emerging Applications to the Physical and Data Sciences long programme.}
\compactentry{Springer Best Student Paper Award, Photonics-2016}{2016}{Nonlinear-Optics session, IIT Kanpur.}
...
\skillline{Simulation and HPC}{eOn, LAMMPS, GROMACS, GPAW context, MPI/OpenMP, CUDA, Slurm, containers, Nix, Spack, EasyBuild, Linux/HPC operations.}
\skillline{Programming}{C++, Python, Fortran, Rust, R, Julia, shell; bindings and ABI work across Python/C++/Fortran/Rust stacks.}
...
\section{Additional Software and Solver Contributions}
\skillline{HiGHS / SciPy / OpenBLAS}{Integrated the HiGHS LP/MIP solver into SciPy (every \texttt{linprog}/\texttt{milp} call), initiated PRIMA derivative-free integration, and ported OpenBLAS to Meson as the default BLAS/LAPACK backend for NumPy and SciPy.}
\skillline{ASV / wailord / fastMatMR}{Co-maintain Airspeed Velocity (Python benchmarking); created an ORCA quantum-chemistry parser/workflow library and rOpenSci-reviewed R bindings for fast Matrix Market I/O.}
